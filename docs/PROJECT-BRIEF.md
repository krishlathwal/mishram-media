...
Header (fixed)
  ↓
Hero — Growth Orbit                     [APPROVED / LOCKED — media re-cast in Rev 42:
                                         Akash Sagar primary, Ali Fazal, Lovekesh
                                         Kataria, Kaka; slot geometry untouched. §10at]
  ↓
...
02 / What We Do                         [Services 01-04 public, 05 hidden but built
                                         ...
                                         Rev 42: scenes re-cast, two buttons per
                                         service, 100vh per service. §10at]
  ↓
The Mishram Difference                  [interlude, unnumbered; rebuilt in Rev 42 as
                                         a static usual-vs-Mishram contrast — §10a, §10at]
  ↓
03 / Creators                           [two layers: the image-backed stage
                                         (four since Rev 42, in the client's order:
                                         Ali Fazal, Akash Sagar, Lovekesh, Kaka — §10at),
                                         + scale facts and the worked-with
                                         index, which now carries verified
                                         profile links. §10b-scale, §10t, §10u]
  ↓
04 / Work Process                       [built; 01–02 renamed Plan / Strategise in
                                         Rev 42 — §10at]
  ↓
...
> **SUPERSEDED IN PART BY §10p (Revision 13) AND §10an (Revision 36).** The architecture, the copy
> and the responsive/reduced-motion behaviour below are all current. **The "CONTENT-BLOCKED" status
> and row 2 of the audit table are not** — the section renders one verified award (§10p).
>
> **Two details below moved in Revision 36.** The single-item composition is now an **8 / 3** split
> rather than 7 / 4, and the chapter's text action sits at the **foot of the museum label** instead
> of under the archive. The count-adaptive behaviour, the hover rules, the caption-is-always-DOM-text
> rule and *"no gold"* are all untouched — and **`globals.css` was not modified**, because the
> treatment was already right and it was the photograph that was wrong. See §10an.
...
> **THE ASSET IS SUPERSEDED BY §10an (Revision 36). The four content constraints below are not —
> every one of them still stands, and one got stronger.** The banner crop described here was
> **retired and deleted**: it is a cut-out composite on a flat lilac field, and five first-party
> 3024×4032 photographs of the same presentation were on the drive the whole time. `06 / Recognition`
> now renders one of them. **The award title, organisation and year are unchanged** and still trace
> to this banner's own display type — nothing new was read off the photographs, because the plaque
> and the trophy plate are both illegible at full resolution, exactly as the media audit predicted.
> **See §10an for the `NUFEW` / `NUFW` discrepancy the photographs exposed.**
...
hotlinked),
`public/media/recognition/nufew-award-presentation-2024-25.webp` (1600×1200, the §06 award — a
**first-party 3024×4032 photograph of the award presentation**, cropped and downscaled, §10an. It
replaced a 775×581 crop of the old deployment's promotional banner, which was **deleted**; the
banner was a cut-out composite on a flat lilac field and the photographs of the same moment were on
the drive all along. **`/about` renders this same file** — the archive board's recognition fragment
and the `#on-the-record` chapter both read `RECOGNITION_ITEMS[0]`, which is why the crop is
authored at exactly 4:3),
...
`public/media/work/swiggy-branded-content.webp` (620×1102) and
`public/media/work/pintola-branded-content.webp` (620×1103) — the two branded-campaign frames in
§05, both first-party and both carrying a legible brand already on the rail (§10am),
`public/media/work/ekly-desktop.webp` (1440×1600, 85.0KB), `public/media/work/ekly-mobile.webp`
(720×2400, 74.2KB), `public/media/work/ruchita-desktop.webp` (1440×2600, 206.8KB) and
`public/media/work/ruchita-mobile.webp` (720×2400, 93.8KB) — **the only captures of third-party live
sites on this website**, both projects supplied by the client as Mishram's own work and both open to
a visitor in one click. Captured over CDP in §10v and trimmed to the region that actually rendered;
**`/services/web-digital-experiences` only**, and each one carries meaningful alt text naming the
site and the width it was captured at (§10ar),
...
- **Revision 42’s decisions (§10at) — the client's own instructions, locked.**
  - **Plum is a surface, reached only through `.plum-field`, on exactly two elements: the
    ...
    never as a canvas tint; never a third surface without the client asking. The scope restates every
    `--t-*` token **and every `--color-*` alias** (§10x); its muted token is .66, not the theme's
    .58, because .58 measured 4.24 : 1 on plum and failed AA.
  - **The award banner is the client's artwork and is never cropped.** From `xl` the chapter is the
    artwork's exact aspect — `calc(1411 / 2560 × 100vw)`, **no cap** (a 50rem cap took the top off the
    wordmark at 1920). Below `xl` it is a block at its own aspect with the plate beneath it. **Do not
    put the plate over the faces**: at 1024 and 1280 a full plate reached the left figure's chin —
    that is why the overlay starts at `xl` and the caption hides at 1280–1439. Nobody in the banner is
    named; `NUFEW` is not expanded.
  - **The stage is the client's ranking — Ali Fazal, Akash Sagar, Lovekesh Kataria, Kaka — and is
    not re-ordered without the client.** Purav Jha and Sagar Rathee lead the index and move to the
    stage **only** with a client-labelled still each; the `.MOV` BTS never becomes one.
  - **Kaka renders as "Kaka".** Not *Immortal Kaka Ji*, not a handle, not a link — none of those is
    established. The unblock is the client supplying the handle, never a search from the face.
  - **The Mirzapur frames name nobody.** *Cast* and *promotional event* come from the client's brief
    and the poster in frame, not from recognising anyone. Keep it one item; it is a highlight.
  - **The Difference chapter is a static table by instruction** — do not reintroduce the rail or the
    connected stack. `SERVICE_SCROLL_VH` is 100 by instruction — do not lengthen it.
  - **Process stages 01–02 are Plan / Strategise; ids are unchanged** — anything keyed on the ids
    still works.
  - **The one-photograph-one-role rule has four documented exceptions** (the four ranked people,
    `MEDIA-ASSET-AUDIT.md` §10). No source may repeat in adjacent chapters; add a fifth exception only
    with the same table entry.
  - **The Hero slot geometry in `layout.ts` is still byte-identical to Revision 17.** Keys and
    exposure bases changed; the numbers that place the cards did not. Keep it that way.
  - **A service-scene capture two or more slots from the sweep's rest needs `after: 5000`** in
    `shoot.mjs`; without it the shot lands on a blank or half-entered scene and looks like a defect.
  - **Not pushed, not deployed, by instruction.** Whoever pushes records a fresh rollback id first
    (§10as).
...
- **Revision 40’s decisions (§10ar) — awaiting review, then locked.**
  - **THE WEB ROUTE'S SEVEN CHAPTERS ARE FINAL.** Hero → Selected Digital Work → What We Build →
    Beyond Websites → Why Mishram → How We Build → Project Inquiry. §10aa closed the information
    architecture, Phase 14 audited every chapter and moved one; **do not add an eighth**, do not
    reorder, and do not reinstate §10v's abandoned four-section tail.
  - **PROOF ARRIVES SECOND ON THIS ROUTE, AND ONLY ON THIS ROUTE.** Two live URLs a visitor can
    open outrank five screens of positioning. Do not "fix" the rhythm to match Services 01–03.
  - **A PROJECT MAY CARRY ITS NAME, CATEGORY, ONE FACTUAL DESCRIPTION, ITS CAPTURES AND ITS URL.**
    Nothing else. The two `note` lines are read off the live sites' own `<title>`, navigation and
    sections, so a reader can check them — **do not extend either into a result, a role, a
    technology, a launch date, a duration, a team size or a testimonial.** Ruchita's own counters
    appear only inside the screenshot of that site and are never repeated in Mishram's voice.
  - **THE NARROW CAPTURE IS AN OBJECT AT EVERY WIDTH.** It was `w-[19%]` — 66px at 390, 0.18× — on
    the one page whose subject is how a site behaves at that width. `w-[32%]` below `sm` matches the
    0.31× the pinned stage already gives it. **Do not shrink it back**, and do not drop it: the pair
    is the responsive claim being shown rather than asserted.
  - **The hang below the window is absolute (`-bottom-20`), not a percentage.** A percentage of a
    container whose height tracks the column reaches 78px at 639px, past the panel's own padding. It
    is 80px rather than 48 because at 48 the phone crossed the Ekly capture's headline — **a capture
    decided that number, so re-look before changing it.**
  - **The project index is `aria-hidden` and nothing on it is clickable**, for the same reason §10
    gives for the homepage progress indicator. Its fill drives `scaleX` from the track's progress
    **directly** — no `useTransform`, because a literal input range is what opts a value into
    Motion's WAAPI path (§10v).
  - **`sizes` on this section is measured, not estimated** (§10aq). `52vw` pinned / `91vw` stacked
    are real box widths; the old `58vw` over-fetched a srcset step at every desktop width and cost
    58KB of cold transfer.
  - **No image on this route is the LCP** — it is text, measured on a production build with no
    scrolling. Next's dev overlay says otherwise and is wrong for the reason §10aq records. **Do not
    add `loading="eager"` on the strength of that warning.**
  - **`Creator Websites` is a category Mishram already serves, not a new promise.** No other
    capability was added: every one on the phase's list was already in `WEB_BUILD_FAMILIES` or
    `WEB_SYSTEM_DIRECTORY`, and **a capability is not created because a brief mentions it.**
  - **1024 and 768 are the longest widths on this route by design**, because `What We Build` and
    `Beyond Websites` both pin from 1200 and a tablet gets their stacked sequences. Lowering either
    threshold is a composition decision for those chapters and needs its own capture pass — **it is
    not a padding problem.**

- **Revision 39’s decisions (§10aq) — awaiting review, then locked.**
  - **THE OVERFLOW OWNER IS `#what-we-do`, NOT THE MARQUEE.** Ten revisions blamed
    `.collab-viewport` because it is the widest box on the page; it is fully contained by
    `.collab-rail { overflow: hidden }` and contributes nothing. The real source is the scenes’
    tilted `.svc-surface` overhang, and **`#what-we-do { overflow-x: clip }` owns it. Do not remove
    that declaration**, and do not re-blame the rail.
  - **`clip`, never `hidden`, on that section.** `overflow-x: hidden` forces the other axis to
    `auto`, which makes the element a scroll container and **breaks the pinned sticky track**.
  - **`body { overflow-x: hidden }` does not clip.** With `html` at `visible` its overflow
    propagates to the viewport and body itself computes to `visible`, so the document still reports
    the overflow. **Never treat it as containment** — that belief is what hid this defect for ten
    revisions.
  - **`scripts/overflow.mjs` is the regression gate.** Run it before claiming any layout fix. It
    prints `innerWidth` beside `clientWidth` deliberately, because `innerWidth` is inflated *by* the
    very overflow it would be used to detect (§10ak).
  - **Consent targets are 48px**, and 44px for the privacy link — this project’s own standard rather
    than WCAG’s floor. **Consent semantics and Consent Mode v2 were not touched.**
  - **No image is the LCP on any route** — it is text everywhere, measured on a production build with
    a buffered observer and no scrolling. Next’s dev overlay reporting the `/about` images as LCP is
    an artifact of the QA scroll sweep. **Do not add `loading="eager"` on the strength of that
    warning.**
  - **`sizes` inside `.svc-stage-box` must be measured, not estimated.** The box letterboxes to a
    fixed aspect, so a `vw` estimate drifts from the real width; two were off by more than 1.5× at
    every breakpoint and were serving files narrower than their own box.
  - **`--color-brand-plum` is deleted.** Five real surfaces across five phases, zero runtime
    consumers. **Do not reintroduce it.**
  - **The 404 is `src/app/not-found.tsx`** — existing primitives only, `noindex`, two routes out, and
    **no invented navigation**. Do not grow it into a sitemap of “popular pages”.
  - **Brand Shoots stays `noindex` and out of the sitemap; Web & Digital stays indexable and in it.**
    Neither was changed here and neither should be changed by a technical pass.
...
- **Revision 36’s decisions (§10an) — awaiting review, then locked.**
  - **`NUFEW` is still never expanded, and the reason is now stronger rather than weaker.** The
    promotional banner's badge reads `NUFEW`; the event's step-and-repeat, the engraved plaque and
    the trophy plate all read **`NUFW`**, and the step-and-repeat prints **`NEXUS UNIVERSE FASHION
    WEEK`** beside it. **That expansion belongs to `NUFW`, not to the string the site publishes** —
    printing them together would assert the two are the same body, which is the open question.
    Publish neither the expansion nor `NUFW`. **It is a one-word client decision**, flagged like the
    Shadab Jakati finding and never resolved on a one-character difference (§10u).
  - **`award.jpg.jpeg` is rejected, permanently.** It is the proposal deck's cover artwork — poster
    typography, a URL, clipart instruments and two cut-out figures — and it carries **no award
    title, badge, year or organisation**. Do not reinstate it, and do not crop it: there is nothing
    in it to crop to.
  - **The two step-and-repeat frames stay held.** Twenty-plus third-party marks on a sponsor wall
    (§9 clean, §18 not), and their plaque is a **differently-scoped award inscribed to an
    individual**. Do not publish either under §06's agency title, and **do not crop the wall away to
    get past it** — the §10aj rule about the OPPO frames, in a new setting.
  - **Nobody in the award photograph is named**, and the library's one presenter-naming *filename*
    is still not evidence (§10p, twice over).
  - **The dominant Recognition item stays 4:3.** `/about`'s `ArchiveBoard` hardcodes that ratio and
    `AboutCredibility` follows `item.aspect`; the aspect is what lets Recognition's asset change
    without editing About. **Change it and re-check both.**
  - **The archive stays at one item.** The three stage frames are the same pose seconds apart — a
    second one is a scrapbook of a single moment, not a second piece of evidence.
  - **`sizes` and the 8 / 3 split are a pair.** Move one and re-measure the other, or the browser
    fetches a candidate narrower than the box (§10q defect 4).
  - **The action lives at the foot of the museum label in the single-item state**, as a slot on
    `RecognitionMedia` — not a fork, and not a second copy of the button.
  - **Plum is rejected on Recognition too**, which was the surface §10af was saving it for. Four
    phases, four real surfaces, no home. Do not test it again without a new argument.

- **Revision 35’s decisions (§10am) — awaiting review, then locked.**
  - **A logo is not a campaign.** A brand on the collaborations rail establishes a relationship and
    nothing more — no specific campaign, deliverable, creator or result. **Canva India is held for
    exactly this reason**: the deck carries its wordmark and handle and no campaign frame. Do not
    create a Selected Work item from a rail entry.
  - **A folder name is a signal; what is in the frame is the evidence.** The Swiggy stills stay held
    because nothing Swiggy is in them; the video frame publishes because the packaging is legible.
  - **40M+ renders once, in 05 / Selected Work, attached to no brand.** The scope note *“Across
    Mishram’s brand collaborations. Not attributed to the work shown here.”* is **not decoration** —
    it is what stops a true agency claim being read as a claim about Swiggy or Pintola beside it.
    Never drop it for space, never shorten the label to “views”, never make it an average, and
    **never add it to the quick-scan proof band** — `public: false` in `config/proof.ts` governs
    that band only.
  - **No Mishram role on any work item** until a source establishes what Mishram handled on that
    specific piece. Agency capability is not evidence of it (§12).
  - **Nobody in a work frame is named**, and no result is attached to any item.
  - **NO SAFE VIDEO SELECTED, and that stands until a clip clears all five rules.** A strong
    still-based index beats the wrong clip. The playback path stays built — `mediaType: "video"`
    plus `src` and nothing else. Do not ship a raw MOV, and do not force video to fill the
    architecture.
  - **Troovy stays on the service page only.** One piece of evidence, one place.
  - **`zoya-jaan.webp` and `nikita-kumawat.webp` are retired from this chapter, not deleted** —
    §03 and the Hero still use them. Do not restore them here to lengthen the index.
...
>
> **§10at (Revision 42):** the client's video walkthrough, implemented. The Hero carries Akash Sagar,
> Ali Fazal, Lovekesh Kataria and **Kaka** (P22, now public); the Creators stage is those four in the
> client's order and the index leads with Fukra Insaan, Purav Jha and Sagar Rathee; **plum is back on
> two surfaces** (the Recognition plate, the Project Inquiry field); **06 / Recognition is the client's
> own award banner, full bleed and never cropped**; **Mirzapur: The Movie** leads Selected Work as film
> PR; the Difference chapter is a static contrast; Process reads Plan / Strategise / Create / Launch /
> Scale; every service has two real buttons. The homepage is **15,469px at 1440** (was 17,787).
...
## 10am. FINAL POLISH PHASE 08 — SELECTED WORK, AND THE FIRST BRANDED CAMPAIGN FRAMES (Revision 35)

**Nothing was deployed.** Production still serves Revision 27. The full audits, the ledger and the
register are in `docs/FINAL-POLISH-ROADMAP.md`; this records the decisions.

### 1 — THE CHAPTER WAS MAKING THE WRONG CLAIM

`05 / Selected Work` carried three items — `zoya-jaan.webp`, `mukul-sharma.webp`,
`nikita-kumawat.webp` — each the **same file already rendering in §03 Creators**, two of them in the
Hero as well. They were honestly labelled and nothing about them was fabricated. They also proved
that Mishram photographs creators, not that it makes work for brands. **On a chapter called Selected
Work that is the wrong claim to be able to make.**

### 2 — THE EXTRACTION GAP THAT HAD HIDDEN THE EVIDENCE

Phase 05 extracted the proposal's imagery by scanning for **JPEG streams**. The deck holds **113
image objects: 41 DCTDecode and 72 FlateDecode** — seventy-two images had never been looked at.

Decoding them found **Pintola**: `img-028`, catalogued in Phase 05 as *"three men, curtain
background"*, is at full resolution a **Pintola® High Protein Muesli** pack presented to camera,
wordmark and pack copy legible — a product-integration frame in the same grammar as the Troovy one,
sitting on **page 6, *BRANDS WE'VE WORKED WITH***. The rest of the Flate set is brand logo lockups,
a cut-out award photograph with an alpha mask (Recognition, Phase 09 — out of scope) and one
letterboxed relationship photo.

**A correction fell out of the same pass.** Revision 33 recorded the published Xbhandesiri
Reel-performance capture as page 3. Traced through the page objects' `/XObject` dictionaries it is
**page 5 — "BRAND VIDEO & VIEWS"**: better provenance than the record claimed, and the asset and
figures are unaffected.

### 3 — THE SWIGGY FRAME PHASE 05 COULD NOT FIND

Phase 05 examined the two **stills** in `PRASHANT VIDEO/Swiggy/` and correctly held them — four
people in a room with nothing Swiggy in shot. That verdict stands for those two files.
...
**Swiggy delivery boxes and the "Food you ♥ on time" tagline legible** in the foreground.

**A folder name is a signal; the packaging in the frame is the evidence.** That distinction is why
this frame publishes and those two stills still do not.

### 4 — NO SAFE VIDEO SELECTED, AND IT IS A RESULT RATHER THAN A GAP

Every campaign-relevant clip was probed and sampled: the four in the Swiggy folder, the sixteen
`REELS - MISHRAM/CREATIVE REEL - ALL` skit folders, and `14th work load/final.mp4`.

**None passes rule 4 — "suitable for a premium agency portfolio."** The in-house reels are Mishram's
own office skits (*"asking for agency"*, *"WHERE IS OUR SOCIAL MEDIA MANAGER"*); the other Swiggy
clips are vlog footage of identifiable private individuals in public places; `final.mp4` stays held
on §10t's three grounds. And **the one clip with real branding makes a better still than motion** —
as video it is a handheld take of people crossing a room.

The playback path is untouched and fully built: `mediaType: "video"` plus `src` switches it on with
no component edit, and there are **zero `<video>` elements in the DOM** today.

### 5 — THE FINAL THREE, AND WHY THE THIRD IS THE ONE IT IS

**01 Swiggy · 02 Pintola · Branded Content, Worked with. 03 Mukul Sharma · Creator Content.**
Branded work leads, so a visitor who reads only the featured state sees a real brand.

The lead promises *creator content, campaigns and visual work*, so dropping creator content entirely
would make the lead wrong. **Mukul is the least-repeated of the three it replaces**: Zoya Jaan and
Nikita Kumawat both render in the Hero *and* §03, so either would be a third appearance on one page;
Revision 28 took Mukul off the Hero. It is still a repeat, and it is the smallest one available.

`WorkItem.relationship` is new and renders only where the collaborations rail already evidences one.
Absent on creator content, and **never upgraded to *managed*, *client* or *campaign by*.**

**Troovy was deliberately not repeated** — it carries the campaign-proof band on
`/services/influencer-marketing`, and a second crop of the same photograph would put one piece of
evidence in two places. **Canva India is held**: the deck has the wordmark and `@canvaindia` and
**no campaign frame anywhere** across all 113 image objects. A logo is not a campaign.

### 6 — 40M+, PUBLISHED ONCE AND ATTACHED TO NOTHING

> **40M+** / **VIEWS ON A SINGLE BRANDED VIDEO**
> *Across Mishram's brand collaborations. Not attributed to the work shown here.*

Above the work index, on the chapter's own hairline. Every word of the proposal's scope survives:
*"on a single video"* — not a total, not an average, not a monthly figure; *"some of our brand
collaborations"* — an agency-level statement about an unnamed collaboration.

**The scope note is load-bearing, not decoration.** Nothing in any source says which collaboration
the figure belongs to, and it renders inches from the words *Swiggy* and *Pintola*. That sentence is
what stops a true agency claim being read as a false client claim.
...
on one page — the duplication Revision 33 removed.

### 7 — WHAT IS NEVER CLAIMED HERE

No campaign name, brief, date, deliverable count or creator list — the sources establish a brand and
a frame, not a project. **No Mishram role**: nothing says who did what, and *"agencies usually do
these things"* is not evidence. **Nobody is named** — neither photograph carries identity metadata,
and §18 rule 7 bars using a face. **No result on any item.** *"Billions of views"* stays held.

Both published brands are on the collaborations rail with an official asset and a client-confirmed
relationship. **No held brand appears in any frame** — zingbus, Fun N Earn, VYRL, Duolingo and all
OPPO material are absent — and **nothing was cropped to get around a prior decision.**
...
| New assets | — | **2, 125.8KB** (52.8 + 73.0) |
| Image nodes · lazy · eager · preload | 23 · 23 · 0 · 0 | **unchanged** |
| `<video>` in `#work` | 0 | **0 — no decoder mounts** |
| Net media requests | — | **0.** Two new posters replace two no longer requested here |

The growth is the 40M+ block. **No copy was cut to pay for it**, and the only thing available to cut
was the scope note — the one line that keeps the claim honest.

**Two posters were retired, not deleted**: `zoya-jaan.webp` and `nikita-kumawat.webp` still serve
§03 and the Hero. Only this chapter's references moved.

### 9 — MOBILE AND REDUCED MOTION

**390: pass.** One item at a time — heading, lead, the 40M+ line, a full-width 9:16 still with the
Swiggy boxes legible, the item's metadata, the three-row index, the CTA. No three-column metadata,
no tiny screenshot, **zero overflowing elements inside `#work`** at 1440, 1024, 768 or 390.

**Reduced motion** is unchanged and correct: every item is a poster, so there is nothing to
autoplay. The clip wipe becomes a short opacity swap and all three items stay selectable.

### 10 — THE CAPTURE SCRIPT LEARNED SOMETHING EXPENSIVE

`scripts/shoot.mjs` gained a **`click`** step, so a selected state comes from the component rather
than from a prop set for a screenshot.

**And the section-capture routine now converges instead of resizing twice.** Growing the viewport to
a section's height reflows the document — and `02 / What We Do` is a `100svh + N×130vh` track, so
**its height is a function of the viewport height.** One resize moved every section below it by
hundreds of pixels, and a capture of `#work` came back showing the inquiry form. It now resizes,
sweeps, re-measures and repeats until the offset stops moving, then **asserts the section actually
landed where the frame claims.**

That assertion immediately flagged every padded seam as `2 × pad` wrong — because it compared
against 0 rather than `pad`. **The check was the bug, not the shot**, which is what an assertion is
for.

### Verified

- **Types, lint and the production build clean.** Twenty routes, all still static.
- **Seventeen real composited captures** — the chapter at eight viewports plus reduced motion, each
  work item in its own selected state, and both seams.
- **`40M` appears exactly once on the rendered homepage**, inside `#work`, with zero occurrences in
  the quick-scan band — audited in the DOM, not in the source.
- **No brand is attached to the figure**, and no result is attached to any item.
- **No dependency, no new analytics event.**
- **Nothing pushed, nothing deployed.**

---

## 10an. FINAL POLISH PHASE 09 — RECOGNITION, AND THE REAL AWARD PHOTOGRAPHS (Revision 36)

**Nothing was deployed.** Production still serves Revision 27. The full audits, the ledger and the
register are in `docs/FINAL-POLISH-ROADMAP.md`; this records the decisions.

### 1 — THE SUPPLIED "AWARD BANNER" IS NOT AN AWARD BANNER

The phase was scoped around `WEBSITE SHORTLIST/award.jpg.jpeg` becoming the chapter's editorial
banner. **It was opened before anything was built with it** — §10p's rule, earning its place for the
sixth revision running — and it is the proposal deck's **cover artwork**: a Canva composite carrying
`MISHRAM MEDIA` at poster scale, *"AN AWARD-WINNING / CREATIVE INFLUENCER & / DIGITAL MARKETING
AGENCY"*, `www.mishram.media`, clipart instruments, and **two cut-out figures with the background
removed**.

**It carries no award title, no `NUFEW` badge, no year and no organisation.** So it is *weaker
evidence than the asset it was meant to replace*, and as a banner it is §10q's defect in a new
costume — a promotional graphic with its own typography pasted onto an editorial page. The plan made
its use conditional on passing real visual QA. It does not. **Rejected, not re-cropped**: there is
nothing in it to crop *to* except a cut-out on a synthetic ground, which the plan also forbids.

The Revision 28 ledger's caveat on this file was exactly right, and its third option was the one
that paid: **the original photographs behind the graphic were already on the drive.**

### 2 — THE HIGHEST-VALUE FINDING IN THE REVISION 17 AUDIT WAS FINALLY SPENT

`docs/MEDIA-ASSET-AUDIT.md` §3B flagged five root HEICs as *"the single highest-value finding in this
audit"* and correctly declined to act on them inside a creator-media revision, recommending a scoped
Recognition revision. **This is that revision.** Five first-party **3024×4032** photographs of the
award evening — four times the resolution of the banner crop, and actually photographs.

**One inventory correction.** `03EBDAA5-….HEIC` has gone from the drive root and **`Award.HEIC` sits
there at exactly 3,265,980 bytes** — byte-for-byte the size the conversion manifest records for it.
The client renamed the file, and the new name is first-party metadata confirming what it is.

They split into two groups: **three stage frames** (two figures, the gold trophy, the ceremony
backdrop, an audience) and **two step-and-repeat frames** (one figure, an engraved plaque, the
event's sponsor wall). All five are natively upright — the Revision 17 Windows-Imaging-Component
pass resolves orientation at decode — so **no `.rotate()` was needed and none was guessed at.**

### 3 — ONE PHOTOGRAPH SHIPS, AND FOUR ARE DELIBERATELY NOT USED

**`186F38BE-…` is published.** Chosen by looking at all three stage frames at the same crop: it holds
the trophy highest and most centrally with both faces to camera.

The other two stage frames are **the same pose seconds apart**. Publishing two would be a scrapbook
of one moment, which the plan rules out, so the archive stays in its count-adaptive **single-item**
state and the chapter argues on one photograph.

**The two step-and-repeat frames are HELD on two independent grounds**, either of which is enough:

1. **The backdrop is a sponsor wall carrying twenty-plus third-party marks.** Every legible one was
   read — VLCC Institute, Prima, Peplos Jeans, Stylox, Satmola, Inkz, Aagaaz Events, 360 Advertising
   Production, Gopal's 56, HB Klyde Premier, D'Vomore, All India News, Samar Salon Academy, Ever
   Pure, MCF, Mr & Ms Next Super Models and more. **§9 was checked against all of them and none is a
   betting, gambling, casino, fantasy-gaming or offshore-CFD brand** — a clean result. But §18's
   *"a third-party brand in frame is a brand claim"* does not survive twenty of them at 946px.
   (The wall also carries **Star Crown Media**, the agency's own 2021 name — good corroboration that
   this is Mishram's own event presence, and not a reason to publish the other twenty.)
2. **The plaque is a different award.** At full resolution it is inscribed to a **digital partner**
   and addressed to an individual by name — not *Best Digital Marketing Agency*, and not to the
   agency. Publishing it under §06's title would attach the wrong artifact to the claim.

### 4 — `NUFEW` vs `NUFW`: A DISCREPANCY RAISED, NOT SETTLED

Reading the photographs at high zoom found a conflict that could not be seen while the only evidence
was a rendered badge:

| Source | Reads |
| --- | --- |
| The promotional banner's gold badge (a *designed graphic*) | **`NUFEW`** |
| The event's own step-and-repeat, many times over | **`NUFW`** |
| The engraved plaque | **`NUFW`** |
| The trophy's plate | **`NUFW`** |

And the step-and-repeat prints the expansion beside the mark: **`NUFW — NEXUS UNIVERSE FASHION
WEEK`.**

**The site keeps publishing `NUFEW`, and it is still never expanded.** It is the string the client's
own material carries; a designer's badge and an engraver's die can disagree without either being the
body's legal name; and `NUFEW` → `NUFW` is a **one-character difference**, which is exactly what
§10u spent a revision learning not to resolve on its own.

**The expansion is not published either**, and that is the load-bearing half: `NEXUS UNIVERSE
FASHION WEEK` expands `NUFW`, and the site prints `NUFEW`. Setting them side by side would assert the
two are the same body, which is the open question. **§18's rule survives this revision intact.**

**One-word decision for the client**, in the same shape as the Shadab Jakati flag.

### 5 — NOTHING NEW IS CLAIMED FROM THE PHOTOGRAPHS

The audit predicted the plaque inscription would still be illegible at full resolution. **It is**,
and so is the trophy's plate — roughly 117×125 usable source pixels, extracted natively and upscaled
before the conclusion was drawn. **No award wording was read off any photograph.** The published
title still traces to the banner's own display type via §10p.

**Nobody is named.** Two figures are in the published frame and neither is identified; no face was
compared, matched or recognised. The one file in the library whose *filename* names a presenter was
refused again on §10p's rule that a filename naming a person is not evidence of a person.

### 6 — THE COMPOSITION, AND WHAT PAID FOR IT

Two changes, both driven by a capture rather than a preference.

**The split went 7 / 4 → 8 / 3.** With one photograph carrying the whole chapter, 824px did not read
as proof and **946px does**. Both were rendered before the choice was made. **`sizes` moved with it —
58vw → 66vw** — because a stale `sizes` is §10q's defect 4, and the two are now commented as a pair
that must be re-measured together.

**The chapter's action moved into the foot of the museum label.** Beside a 618px photograph the label
bottomed out around 180px and left half a column of empty canvas — invisible to every measurement
and obvious in the first capture (§10q's lesson again). Anchoring the action to the column's foot
finishes the label *and* takes its own band out of the section, **which is what paid for the wider
frame**. Implemented as a **slot on the shared primitive, never a fork** (§18): `RecognitionMedia`
gained an `action` prop, passed only in the single-item state.

**`globals.css` was not touched at all.** §10e's `.rcg-*` treatment was already right for a real
photograph; it needed a better photograph, not a change.

### 7 — NO GOLD — AND THIS TIME NONE ARRIVES THROUGH THE IMAGE
...
rosette, gold clipart stars and a flat lilac field onto an obsidian page — the palette was leaving
via the asset. The published photograph's only gold is **the trophy itself, in shot**, which is
exactly where the plan says any gold tone should come from.

### 8 — PLUM: TESTED ON THE SURFACE IT WAS BEING SAVED FOR, AND REJECTED

§10af named *"a Recognition surface (§09)"* as one of three legitimate future homes, and it is the
strongest of the three — an award chapter is the page's one genuine event surface. Option B, the
whole section as a plum field with ivory content, was rendered in both themes and at the seam.

**Rejected.** The seam is a hard flat slab with no transition (as §10ah found); the field pulls a
purple cast across a photograph whose only warm note is the trophy; and it **re-creates the exact
defect this revision removed** — a flat lilac panel dominating an obsidian page, now at section scale
with the real photograph inside it, so the *page* becomes the advertisement instead of the image.

**Four phases have now tested plum on real surfaces and none has found one.** This is the strongest
refusal of the four, because it is the surface the token was being held for. The test CSS was
removed; the diff carries no `!important` and no test rule.

### 9 — MEASURED

| | Before | After |
| --- | --- | --- |
| `#recognition`, 1440×900 | 1,232px | **1,239px (+7, 1.38 vh)** |
| `#recognition` at 1280 / 1024 / 768 / 430 / 390 | 1,152 / 1,021 / 1,255 / 888 / 858 | **1,150 / 1,003 / 1,235 / 876 / 846 — shorter at every one** |
| **Homepage, 1440×900** | 17,779px | **17,787px (+8, +0.05%)** |
| Homepage at 1280 / 1024 / 768 / 430 / 390 | 16,742 / 13,987 / 17,457 / 18,413 / 18,082 | **16,739 / 13,969 / 17,437 / 18,401 / 18,070** |
| `/about` | 10,731px | **10,731px — byte-identical** |
| Production asset | 775×581, 86.9KB | **1600×1200, 303.6KB** |
| Image nodes · lazy · eager · preload | 23 · 23 · 0 · 0 | **unchanged** |
...
viewports.** The delivered increase is the honest cost of the swap — a flat three-colour graphic
compresses to 21KB and a high-ISO night photograph does not — and it buys a genuine fix: the old
**775px source was rendering into an 824px box, so the browser was upscaling it**, which is why the
award "came back soft". The new one is downscaled at every viewport. Still below the fold, still
lazy, still no preload, and still the only image in the chapter.

### 10 — `/about` CHANGED WITH IT, DELIBERATELY AND WITHOUT BEING EDITED

`AboutCredibility` and `ArchiveBoard` have both read `RECOGNITION_ITEMS[0]` since Revision 15, so
this asset is upgraded on that route too. **`ArchiveBoard` hardcodes a `4 / 3` recognition fragment**,
which is why **the crop was authored at exactly 4:3**: holding the ratio is what let the photograph
improve on two routes with **no edit to About**, which was out of scope. Both were captured to
confirm it, and the route's height is byte-identical. Anyone changing the dominant item's aspect
must re-check both.

### Verified

- **Types, lint and the production build clean.** Twenty routes, all still static.
- **Thirty real composited captures** — 16 after-state (the chapter at eight viewports plus reduced
  motion, the evidence frame, both seams, and `/about`'s two consumers), 7 baseline, 3 plum A/B, and
  4 intermediate, one per composition decision.
- **Every candidate was looked at, not inferred** — the banner, all five photographs, the plaque, the
  trophy plate and the full sponsor wall, each extracted at native resolution first.
- **Zero overflowing elements inside `#recognition`** at any viewport; the document-level overflow at
...
## 10aq. FINAL POLISH PHASE 12 — RESPONSIVE, ACCESSIBILITY AND TECHNICAL HARDENING (Revision 39)

**Nothing was deployed.** Production still serves Revision 27. The full matrices and the deferral
ledger are in `docs/FINAL-POLISH-ROADMAP.md`; this records the decisions.
...
## 10ar. FINAL POLISH PHASE 14 — WEB & DIGITAL EXPERIENCES, THE DEEP POLISH (Revision 40)

The service page §10o held back for a dedicated pass, and the one route where **the site itself is
the portfolio**. This phase audited all seven chapters, changed one, and added one row to another.

### 1 — THE AUDIT, AND WHY SIX CHAPTERS DID NOT MOVE

Every section was inspected at ten viewports in both themes and under reduced motion before anything
was edited — height, purpose, interaction, copy, media, CTA, mobile behaviour — along with
`config/service-web.ts`, the six route-only components, the shared service primitives and homepage
Service 04.

| Chapter | Verdict |
| --- | --- |
| `01` Hero — the Digital Build Stage | **Unchanged.** Four surfaces at four depths over a measurement field. CSS, DOM and Motion only — **no second WebGL scene** (§12), no browser chrome (§10) |
| `02` Selected Digital Work | **The one underpowered section.** See §2 |
| `03` What We Build | **One row added.** 33 → 34 capabilities in three families, still an editorial matrix on hairlines — no icons, no cards, no bento grid |
| `04` Beyond Websites | **Unchanged.** The `.web-ink` chapter, the four-state architecture, the `Custom CRM` callout and the 25-item custom-systems directory |
| `05` Why Mishram | **Unchanged.** One drawing, drawn once — the route's exhale before the form |
| `06` How We Build | **Unchanged.** Six stages, six artifacts, the stack statement, the capability index |
| Project Inquiry | **Unchanged.** Shared section, shared form, shared endpoint, `web` preselected |

**Nothing was rebuilt, no section was added or removed, and the order did not change.** §10aa's
information architecture is still closed and this phase did not reopen it.

### 2 — THE DEFECT: A 66PX PHONE ON A PAGE ABOUT RESPONSIVE DESIGN

`02 / Selected Digital Work` renders two live projects, each as a wide capture in a hairline window
with the narrow capture overlapping it. On the stacked path that narrow frame was `w-[19%]` of the
column — **66px at a 390 viewport, 51px at 320**.

A 720px capture of a 360-CSS-px layout at 66px is **0.18×**. The wide capture beside it renders at
**0.24×**. So the more legible artefact of the two, per pixel of column, was the smaller of the two —
on the one page whose whole subject is how a website behaves at that width.

**The layout was correct, which is why no measurement found it.** §10q's lesson for the fifth time:
geometry cannot clear a composition.

**Fixed by art direction, not redesign.** Below `sm` the narrow capture is `w-[32%]` capped at 132px
— **111px / 0.31× at 390**, the same scale the pinned stage already gives it at 1440. Above 640
nothing moved: 19% of a 698px column is already 133px. Two details, both settled by looking:

- **The hang is absolute, not a percentage.** `-bottom-[7%]` of a container whose height tracks the
  column reaches 78px at 639px — past the panel's own padding and into the next project's head.
  `-bottom-20` is the same 80px at every narrow width, so one `pb` clears it.
- **80px, and a capture chose it.** At 48px the phone's top edge crossed the Ekly capture's own
  headline. At 80px it sits 21–38% down, the wide view keeps its proposition legible, and the narrow
  view carries **the same words** at a readable size. Complementary, not one hiding the other.

### 3 — THE PORTFOLIO IS DOCUMENTED NOW, NOT SCREENSHOTTED

**Each project carries a one-line description, and every word is read off the live site.**
`WorkProject.note` has existed since §10v under a written rule — *a description of what the site is
is legitimate; results, stack, timeline or Mishram's role are not.* Both are filled from the sites'
own `<title>`, navigation and sections, so a visitor can open the URL and check the sentence:

> **Ekly** — *"A product and marketing site for an AI generation studio — pricing, guides, editorial
> and the route into the app."*
>
> **Ruchita Interiors** — *"A studio site for an interior design practice — services, a project index
> with case pages, editorial and enquiry."*

**The narrow capture has real alt text.** It was `alt=""` while it was a 66px decoration; an object
describes itself. The two strings differ only in the width they name.

**A provenance line joined the closing rail** — *"Captured from the live sites — desktop and
mobile."* Every other piece of evidence on this site carries one and this section had none. It
shares the rail with `Design · Build · Live`, so above `md` it **costs no height at all**.

**A project index sits at the foot of the pinned panel.** A reframing stage cannot say how many
projects there are; somebody who scrolls quickly past the first can leave believing the portfolio is
one site. It is the same device the homepage's service track uses — indices on a hairline with an
accent fill — **non-interactive**, and `aria-hidden` because every project's number, name, category,
note and link are already real text in both slots. The fill is the track's progress driving `scaleX`
**directly**, with no `useTransform`: a literal input range is what opts a value into Motion's WAAPI
path (§10v).

### 4 — `CREATOR WEBSITES`, AND WHAT WAS NOT ADDED

Mishram's own business is established Indian content creators (§1), `03 / Creators` is a whole
homepage chapter, and the Websites family listed `Personal Brand Sites` — which names the outcome
without naming the client. **A creator arriving on this page had to infer that Mishram builds for
creators, on the site of a creator agency.** One row, in the family it belongs to.

**Nothing else was added.** The full capability list this phase was given was checked against the
config item by item, and every other category — static, dynamic, business, corporate, portfolio,
personal brand, landing pages, campaign sites, e-commerce, custom web applications, custom CRM,
internal tools, dashboards, custom software, workflow automation, integrations and mobile
applications — was **already published**, in `WEB_BUILD_FAMILIES` or `WEB_SYSTEM_DIRECTORY`. **No
capability was created because a brief mentioned it.**

Re-measured after the row landed: **34 rows, zero wrapped, at 320 / 360 / 390 / 768 / 1200 / 1440.**
§10w's defects 2 and 3 do not recur.
...
**Worth 58KB of cold desktop transfer.** Neither of the four service-scene cases §10aq deferred is
on this route — the page holds **four images in total** — so nothing was broadened into another
service.

### 6 — MEDIA

**No new asset was produced and no capture was taken.** The four files already existed from §10v and
are byte-identical on disk: `ekly-desktop.webp` 1440×1600 / 85.0KB, `ekly-mobile.webp` 720×2400 /
74.2KB, `ruchita-desktop.webp` 1440×2600 / 206.8KB, `ruchita-mobile.webp` 720×2400 / 93.8KB. What
changed is the size two of them are rendered at and the `sizes` hint that selects a candidate.

**Ruchita's own hero publishes counters.** They appear only inside the screenshot of that site and
are never repeated in Mishram's voice — §10v's rule, re-verified against the rendered DOM.
...
`#digital-work` at 390: **1,336 → 1,661**. **The desktop delta is +9px and the phone delta is +357,
and that asymmetry is the design** — on a desktop the notes sit in the meta column's own headroom
and the provenance line shares the closing rail, so both are free.

**The homepage is byte-identical at every viewport**: 17,787 / 16,739 / 13,969 / 17,437 / 18,401 /
18,070 — the Revision 38 numbers to the pixel.

**1024 and 768 are the longest widths on this route and always have been**, because `What We Build`
and `Beyond Websites` both pin from 1200 and a tablet gets their stacked sequences — 9,668px between
them, 55% of the route. **Not changed:** it is a composition decision for two chapters this phase
was not auditing. Recorded so it is not re-derived.

### 8 — PERFORMANCE, LOCAL LAB DATA

Both states built for production and served with `next start`, cold cache, **no scrolling**, mobile
throttled 4× on CPU, LCP/CLS from an observer registered *before the document*. Two samples each.
...
**CLS is 0 in all eight readings and the LCP element is text everywhere.** Next's dev overlay again
reported an image as LCP and asked for `loading="eager"`; §10aq already recorded that as an artifact
of the QA scroll sweep, and the production measurement is the control. **No eager image, no
preload** — the route runs 4 images, 4 lazy, 0 eager, 0 preload.

### 9 — VERIFIED

- **Types, lint and the production build clean.** Twenty routes, all still static.
- **`PASS — 120/120`** on the overflow matrix: 10 routes × 12 viewports.
- **Console clean** at 1440, 1024, 768, 390, 320 and under reduced motion — zero application errors,
  zero hydration warnings, zero failed resources. Three messages, all third-party or dev-only.
- **Accessibility**: one `h1`, `h1 → h2 → h3` with no skipped level and identical at 1440 and 390,
  every image with `alt`, both project links `rel="noopener noreferrer"` with distinguishable
  accessible names, the off-screen project still `inert`, `.web-visit` still 44px.
- **Numeric integrity**: every figure on the rendered route is an index, the published phone number
  or an inquiry-form option. Zero hits for the banned register.
- **Visibility unchanged** — Web & Digital public, indexable and in `sitemap.xml`; **Brand Shoots
  still `noindex, nofollow` and absent from the sitemap.**
...
## 10at. REVISION 42 — THE CLIENT'S WALKTHROUGH BATCH

**The client recorded a 5:23 screen walkthrough of the homepage (`Video (5).mp4`, 6 September 2026)
and shared `Mishram web data\` — the award banner, a Mirzapur promotional still and a clip. This
revision implements that video.** It touches eleven chapters of the homepage, adds nine production
files, brings the brand plum back on two surfaces, publishes Kaka, leads Selected Work with film PR,
and takes 2,318px off the desktop page. **Committed locally on `main`. Not pushed. Not deployed.**
The media record is `MEDIA-ASSET-AUDIT.md` §10; the phase record with every measurement is the
roadmap's Revision 42 section. This section is the decisions.

### 1 — THE SOURCES, AND HOW THEY WERE READ

The video was transcribed on this machine with Whisper (large-v3-turbo, quantised; nothing left the
computer) and the transcript read against frames pulled every few seconds, so each instruction is
tied to what the cursor was pointing at. Spoken names arrive through an accent and a speech model,
so **every name was reconciled against the project's own records before anything was published** —
never against a face:

| Spoken / written | Resolved to | How |
| --- | --- | --- |
| *Faizal Ali*, *Fazlali*, *Pazilali* | **Ali Fazal** | The only Ali in the library and the roster; `ali fazal.jpeg` |
| *Lovkataria*, *Lovekhatariya*, *Labkataria* | **Lovekesh Kataria** | `Lovekesh Kataria.jpeg`; `WORKED_WITH` |
| *Pandey Shirish*, *Bhandeshiri*, *Bhande Shri* | **Akash Sagar (`@xbhandesiri_`)** | The handle's display name, verified in Rev 17; the client calls him by it |
| *Purab Jahan*, *Puraav Jha*, *Purabja* | **Purav Jha** | `WORKED_WITH`, `@puravjha` verified Rev 17B |
| *Sagar Rathi*, *Sagarrati* | **Sagar Rathee** | `WORKED_WITH`, `@dr.69___` verified Rev 17B |
| *Pukhrainshan*, *Pukarain's on* | **Fukra Insaan** | `WORKED_WITH`; *"to the top"* of the index |
| *Manish Jain*, *JJ Communication* | **Manish Jain · JJ Communications** | One person, one company — one index row, as Rev 34 recorded |
| *Mukesh Jain*, *Sallu Nish*, *podcast* | **Mukesh Jain · Shallu Nisha Podcast** | One person, one show — one index row |
| *Kaka, the singer* | **Kaka** — `Immortal Kaka Ji.jpeg` | The only Kaka in the library; the client's own word for him. **Rendered as "Kaka"**, the form both sources share; the file's longer form is recorded, not published |

**The "newly added creator folder" does not exist.** `C:\`, `F:\` and Google Drive were searched;
the only client-labelled photography is still `WEBSITE SHORTLIST\` (fifteen files). The instruction's
*Kaka has still not been added* resolves against that folder, and the spoken instruction is the
relationship record P22 waited for since Revision 34.

### 2 — THE RANKING, AND WHERE EACH PERSON IS

The client's order, in their words, was Ali Fazal, Lovekesh Kataria, Akash Sagar (*"most
important"*), Purav Jha, Sagar Rathee, Manish Jain / JJ Communication, Mukesh Jain / Shallu Nisha
Podcast, then Kaka — with the four they did not name (Zoya Jaan, Nikita Kumawat, Mukul Sharma, Vishnu
Priya) moved *"down to here"*, the index.

| Priority | Name | Stage | Hero | Scenes | Index | Why not higher |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Ali Fazal | **01** | 4:5 | Service 01 | — | — |
| 2 | Lovekesh Kataria | **03** | 4:5 (new) | Service 01 | — | — |
| 3 | Akash Sagar | **02** + Current Management | **primary 9:16** (new) | Service 02 | — | — |
| 4 | Purav Jha | — | — | — | **lead 02** | **No still exists.** `.MOV` BTS only; nobody in it can be named (rule 7) |
| 5 | Sagar Rathee | — | — | — | **lead 03** | Same |
| 6–7 | Manish Jain · JJ Communications | — | — | — | row 04 | `JJ Communication.jpeg` is an OPPO store — blocked since §10t |
| 8–10 | Mukesh Jain · Shallu Nisha Podcast | — | — | — | row 05 | No photograph |
| 11 | Kaka | **04** | 1:1 (new) | Service 01 | unverified list | Handle not supplied; rule 7 forbids finding it from a face |
| — | Fukra Insaan | — | — | — | **lead 01** | *"to the top"* — a link, no still |
| — | Zoya Jaan, Nikita Kumawat, Mukul Sharma, Vishnu Priya | off | Nikita faint (desktop) | 02 / 04 / 02 / 03 | rows | Moved down by instruction; they carry the scenes the ranked four would otherwise repeat in |

**Spread, not stacking.** With one photograph per ranked person, prominence means recurrence, so
the recurrence is placed: Hero → Service 01 → stage for Ali, Lovekesh and Kaka; Hero → Current
Management → Service 02 → stage for Akash — never two adjacent chapters, and no single crop rendered
twice except Lovekesh's 3:4 (Service 01 and the stage, with the Difference chapter between them).
`config/creators.ts` now exports `STAGE` (the four) and `ROSTER` (the eight published); the stage
reads `STAGE`, the social service page's field reads `ROSTER`.

### 3 — THE HOMEPAGE, CHAPTER BY CHAPTER

| Chapter | Change |
| --- | --- |
| **Hero** | Surfaces: Akash Sagar (primary 9:16, *Current Management*), Ali Fazal, Nikita Kumawat (faint, desktop), Lovekesh Kataria (4:5, *Creator Network*), Kaka (1:1, *Worked With*). **Slot geometry byte-identical**; keys renamed, exposure bases re-tuned. `HeroStatic` mirrors it |
| **Collaborations / Current Management / Quick Proof / About** | Padding only — the *"so much blank space"* pass |
| **What We Do** | `SERVICE_SCROLL_VH` 130 → 100. Two real buttons per service (*Explore service* filled, *Discuss this project* outlined) — *"buttons are so light"*. Scenes re-cast: 01 Kaka / Lovekesh / Ali; 02 Nikita / Zoya / Akash / Mukul; 03 gains a real photograph (Vishnu Priya) in the creative frame; 04 unchanged. Copy holder 23.5rem so the second button never clips |
| **Difference** | The animated rail and connected stack are gone; a static two-column contrast — *The usual setup* / *With Mishram* — four rows. `config/difference.ts` rewritten |
| **Creators** | The stage is the four ranked; the index leads with Fukra Insaan, Purav Jha, Sagar Rathee, and its rows follow the ranking; Zoya Jaan and Mukul Sharma join it without handles (`WORKED_WITH_UNVERIFIED`) |
| **Work Process** | 01 **Plan**, 02 **Strategise** — *"first we'll plan the content, then we'll strategise"*; 03–05 already read Create / Launch / Scale. Ids unchanged, descriptions and activities re-written to the names |
| **Selected Work** | **Mirzapur: The Movie · Film PR · PR support** leads. Lead copy: *"Selected film PR, brand campaigns and creator content from across our network."* `WorkItem.support` lets the support crop be its own frame |
| **Recognition** | See §5 |
| **Project Inquiry** | A plum field — see §4 |

### 4 — PLUM, BACK ON THE CLIENT'S INSTRUCTION

`#4c3660` was declared in Revision 28, rendered on five surfaces, rejected on each and removed in
Revision 39. **The client asked for it; it returns as what the arithmetic always said it was — a
surface, never an accent.** One scope, `.plum-field`, restates every `--t-*` token *and every
`--color-*` alias* (§10x) at plum values, and two elements carry it:

1. **The Recognition plate** — the banner is plum-typed artwork; a plum plate reads as part of it.
2. **The Project Inquiry field** — the page's closing conversion moment, set apart from seven
   obsidian chapters by the one colour change on the page.

Ink on plum 9.14 : 1, soft 5.83 : 1, accent 6.42 : 1; **the muted token is raised to .66 inside the
scope (5.00 : 1) because the theme's .58 measured 4.24 : 1 on plum and failed AA for the small caps.**
No plum text, line, label, icon, gradient or canvas tint anywhere. Two surfaces, and deliberately no
third — the walkthrough asked for a colour, not a purple site.

### 5 — THE AWARD BANNER

*"Remove this section and add just that banner which I created in Canva, and put only that — other
than that you can add the titles."* Revision 36 rejected `award.jpg.jpeg` as a composite marketing
graphic; **the client's instruction overrides that**, and the same title, organisation and year §10p
read off the badge sit over it on the plum plate. `/about` still renders the Revision 36 photograph.

Three layout facts, each captured before it was decided:

- **From `xl` the chapter is the artwork's exact aspect** — `min-height: calc(1411 / 2560 × 100vw)`,
  no cap. The artwork has type at both edges (the wordmark in its top fifth, the URL line at its
  foot); a 50rem cap took the top off the wordmark at 1920.
- **Below `xl` the banner is a block at its own aspect with the plate beneath it.** At 1024 and 1280
  a readable plate reached the left figure's face; a 4:3 crop cut the wordmark's first letter on a
  tablet. Neither is acceptable for the client's own artwork.
- **The plate sits low, under the two faces and over the tagline corner** — `max(30rem, 28vw)` wide
  from 1440 so the tagline is covered whole (at 1920 a 30rem plate left one ampersand showing);
  its caption line hidden at 1280–1439 only, where the chapter is 705px tall.

Nobody in the banner is named; `NUFEW` is not expanded; the site adds no gold.

### 6 — MIRZAPUR

One still (`DSC04338.jpg.jpeg`, two figures before the film's poster) and one frame from the clip
(2.6s, four figures) — a 9:16 primary and a 4:5 support, in the existing Selected Work frame, as the
first item. **Small on purpose**: the instruction asked for a highlight, not a chapter. The alt says
*members of the cast* because the client's brief says *star cast imagery*, and *promotional event*
because the poster wall is in frame — **no one is named, no one was recognised.** Garment graphics in
frame were inspected at full resolution and cleared under §8's rule. The clip is not played back:
5.28 seconds of handheld footage says less than the still; the `mediaType: "video"` path stays built.

### 7 — NOT DONE, AND WHY

| | Reason | Unblock |
| --- | --- | --- |
| Purav Jha, Sagar Rathee on the stage | No still; BTS footage names nobody | A client-labelled still each |
| JJ Communication photograph | OPPO store interior — a brand claim | A different photograph |
| *"Eight to ten reels"* with brand logos in Selected Work | No reel or logo files shared | The files |
| Kaka's profile link | Not supplied; not searched for (rule 7) | The handle |
| Anything from the proposal deck's network slide | Unchanged: P18 stays HELD | Client confirmation per name |

### 8 — QA

`scripts/shoot.mjs` over CDP against the dev server: **121 captures** across five sets — every
changed chapter at 1440 / 1280 / 1024 / 768 / 430 / 390, dark and light, reduced motion where a
chapter animates, seams, the four scenes active and the stacked mobile services, the Selected Work
items by real click, `/about` and the social service page — **reviewed as images.** Three layout
defects were found by the captures and fixed before the commit (§5). `overflow.mjs --quick`:
**PASS 40/40.** `tsc` clean, `eslint` 0 errors (2 pre-existing warnings), `next build` clean.

| Viewport | Rev 41 | Rev 42 | Δ |
| --- | ---: | ---: | ---: |
...
Images 23 → 27 desktop, 19 → 22 mobile; 0 eager, 0 preload.

### 9 — GIT

One commit on `main`, **not pushed, not deployed**, as instructed. Every changed file is listed in
